...
namespace nsK2EngineLow {
	...
	{
	public:
		using OnDeadEventFunction = std::function<void()>;
		/// <summary>
		/// コンストラクタ。
		/// </summary>
		EffectEmitter();
		...
		void AddDeadEventFunction(OnDeadEventFunction& event)
		{
			m_onDeadEventFunctions.emplace_back(&event);
		}
		void RemoveDeadEventFunction(std::function<void()>& event)
		{
			auto itFind = std::find(m_onDeadEventFunctions.begin(), m_onDeadEventFunctions.end(), &event);
			if (itFind != m_onDeadEventFunctions.end()) {
				m_onDeadEventFunctions.erase(itFind);
			}
		}
	private:
		
		Effect m_effect;					//エフェクト。
		std::list<OnDeadEventFunction*> m_onDeadEventFunctions;
